...
public class Bitacora implements Serializable {
	...
	public static final String MSG_RANGO_FECHAS_MAYOR_A_30_DIAS = "El rango de fechas no debe ser mayor a 30 días";
	public static final String MSG_FECHA_INICIO_MAYOR_A_LA_ACTUAL = "La fecha de inicio debe ser menor a la fecha de fin";
	...
	private void validarHorarioSalida(LocalDateTime fechaHoraSalida) {
		...
		if (fechaHoraSalida.getDayOfWeek().equals(DayOfWeek.MONDAY)
			...
			if ((fechaHoraSalida.getHour() > 19 && fechaHoraSalida.getMinute() > 30)
					|| (fechaHoraSalida.getHour() == 20 && fechaHoraSalida.getMinute() == 30
							&& fechaHoraSalida.getSecond() > 0))
				throw new RuntimeException(MSG_HORARIO_SALIDA_NO_ADMITIDO);
		}

		if (fechaHoraSalida.getDayOfWeek().equals(DayOfWeek.SATURDAY))
			if ((fechaHoraSalida.getHour() > 12 && fechaHoraSalida.getMinute() > 30)
					|| (fechaHoraSalida.getHour() == 13 && fechaHoraSalida.getMinute() == 30
							&& fechaHoraSalida.getSecond() > 0))
				throw new RuntimeException(MSG_HORARIO_SALIDA_NO_ADMITIDO);
	}
	...
	public List<RegistroEntradaSalida> obtenerRegistrosAbiertos() {

		return registroDAO.getRegistrosSinSalidaRegistrada();

	}

	public List<RegistroEntradaSalida> obtenerRegistrosEntreFechas(String fechaInicio, String fechaFin, String placa)
		...
		LocalDate fechaInicioLocalDate = LocalDate.parse(fechaInicio, DateTimeFormatter.ofPattern("dd-MM-yyyy"));
		LocalDate fechaFinLocalDate = LocalDate.parse(fechaFin, DateTimeFormatter.ofPattern("dd-MM-yyyy"));

		// Se agrega un día a la fecha fin para que se incluya en la consulta
		fechaFinLocalDate = fechaFinLocalDate.plusDays(1);

		return registroDAO.getRegistrosEntreFechas(fechaInicioLocalDate, fechaFinLocalDate, placa);
	...
	private void validarRangoFechas(String fechaInicio, String fechaFin) {
		...
		int diasDiferencia = (int) fechaInicioLocalDate.until(fechaFinLocalDate, ChronoUnit.DAYS);

		if (fechaInicioLocalDate.isAfter(LocalDate.now())) {
			throw new RuntimeException(MSG_FECHA_INICIO_MAYOR_A_LA_ACTUAL);
		}

		if (diasDiferencia > 30) {
			throw new RuntimeException(MSG_RANGO_FECHAS_MAYOR_A_30_DIAS);
		}
	}
